...
```
bsp process [--project PATH] [--pdfs PATH] [--pattern GLOB] [--no-turbo] [--company KEY] [--account KEY] [--data {parquet,database,both}] [--export-data {parquet,database}] [--export-format {excel,csv,both}] [--export-type {full,simple}] [--no-export] [--no-copy]
```
...
| `--export-data` | `database` | Data source for export (default: 'database'). Choices: `parquet`, `database`. |
