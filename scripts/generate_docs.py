...
def generate_bank_config() -> str:
    ...
    w("    [new bank request](https://github.com/boscorat/bank_statement_parser/issues/new?template=new-bank-request.yml)")
    w("    on the issue tracker — please attach an anonymised statement to help us")
    w("    build and test the configuration.")
    w()
    w("## Overview")
    w()
    ...
    w("| `STD_TRANSACTION_PAYMENTS_IN` | lines | numeric | Yes | Credit amount per transaction |")
    w("| `STD_TRANSACTION_PAYMENTS_OUT` | lines | numeric | Yes | Debit amount per transaction |")
    w()
...
def generate_exports_guide() -> str:
    ...
    w("| --- | --- |")
    w("| `single` (default) | A single flat transactions table joining all dimensions. Best for spreadsheet analysis. |")
    w(
    ...
    w()
    w("```bash")
    w("# Default: export single preset in both CSV and Excel from database")
    w("bsp process --pdfs ~/statements")
    w()
    w("# Export multi star-schema tables as CSV only")
    w("bsp process --pdfs ~/statements --export-type multi --export-format csv")
    w()
    ...
    w("| --- | --- | --- | --- |")
    w("| `--export-type` | `single`, `multi` | `single` | Export preset |")
    ...
    w("import bank_statement_parser as bsp")
    w()
    w("# Export single CSV from database backend (default project)")
    w("bsp.db.export_csv()")
    w()
    w("# Export multi star-schema tables to Excel")
    w("bsp.db.export_excel(type='multi')")
    w()
    w("# Export JSON")
    w("bsp.db.export_json()")
    w("")
    ...
    w("| --- | --- | --- |")
    w("| `single` | `reporting/data/single/transactions_table.csv` | Flat transaction table |")
    w(
